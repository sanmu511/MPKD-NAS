...
class Auxiliary_Classifier(nn.Module):
    def __init__(self, config):
        ...
        self.extractor1 = nn.Sequential(*[self.position_multi,self.transformer_encoder_multi,self.layer_norm])
    ...
    def forward(self, x):
        ...
        for i in range(len(x)):
            ...
            if idx == 1:
                ...
                out = extractor_layers[-1](x[i] + x[i])  # 最后一层的输出结果
...
class Transformer_Auxiliary(nn.Module):
    def __init__(self, config):
        super(Transformer_Auxiliary, self).__init__()
        self.backbone = Transformer(config)
        self.auxiliary_classifier = Auxiliary_Classifier(config)

    def forward(self, x, grad=False):
        logit,feats = self.backbone(x, is_feat=True)
        if grad is False:
