...
class Transformer(nn.Module):
    def __init__(self, config):
        ...
        # 使用自定义的 TransformerEncoderLayer，并为每层添加量化
    ...
    def fuse_model(self):
        # 融合操作，用于量化
        for encoder in [self.transformer_encoder_1, self.transformer_encoder_2, self.transformer_encoder_3, self.transformer_encoder_multi]:
            for i in range(len(encoder.layers)):
                torch.quantization.fuse_modules(encoder.layers[i], ['self_attn', 'linear1', 'linear2'], inplace=True)
                torch.quantization.fuse_modules(encoder.layers[i], ['norm1', 'norm2'], inplace=True)
