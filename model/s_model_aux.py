...
class Transformer(nn.Module):
    ...
    def _init_encoders(self):
        encoder_layer = nn.TransformerEncoderLayer(d_model=self.config.dim_model, nhead=self.config.s_num_head, dim_feedforward=self.config.s_forward_hidden, dropout=self.config.dropout)
        self.transformer_encoder_1 = nn.TransformerEncoder(encoder_layer, num_layers=self.config.s_num_encoder)
        self.transformer_encoder_2 = nn.TransformerEncoder(encoder_layer, num_layers=self.config.s_num_encoder)
        self.transformer_encoder_3 = nn.TransformerEncoder(encoder_layer, num_layers=self.config.s_num_encoder)

    def update_architecture(self, arch_config):
        # 更新每层的架构配置
        for i, layer_name in enumerate(['encoder1', 'encoder2', 'encoder3']):
            cfg = arch_config[layer_name]
            encoder_layer = nn.TransformerEncoderLayer(
                d_model=cfg['dim_model'],
                nhead=cfg['num_heads'],
                dim_feedforward=cfg['mlp_ratio'] * cfg['dim_model'],
                dropout=self.config.dropout
            )
            setattr(self, f'transformer_encoder_{i+1}',
                    nn.TransformerEncoder(encoder_layer, num_layers=cfg['layer_num']))
        
        # 更新多通道融合层
        cfg = arch_config['encoder_multi']
        encoder_layer_multi = nn.TransformerEncoderLayer(
            d_model=cfg['dim_model'] * 3,
            nhead=cfg['num_heads'],
            dim_feedforward=cfg['mlp_ratio'] * cfg['dim_model'],
            dropout=self.config.dropout
        )
        self.transformer_encoder_multi = nn.TransformerEncoder(
            encoder_layer_multi, num_layers=cfg['layer_num'])
        ...
        encoder_layer_multi = nn.TransformerEncoderLayer(d_model=config.dim_model * 3, nhead=config.s_num_head,dim_feedforward=config.s_forward_hidden, dropout=config.dropout)
