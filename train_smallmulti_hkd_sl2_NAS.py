...
def train(save_all_checkpoint=False):
    ...
    arch_params = {
        'encoder1': {
            'dim_model': [64, 128, 256],  # 可选隐藏层维度
            'num_heads': [4, 8],  # 可选注意力头数
            'mlp_ratio': [2.0, 3.0, 4.0],  # 可选MLP扩展比例
            'layer_num': [2, 3, 4]  # 可选层数
        },
        'encoder2': {
        ...
        'encoder_multi': {
            'dim_model': [192, 384, 768],  # 三个encoder的维度之和
            'num_heads': [4, 8],
            'mlp_ratio': [2.0, 3.0, 4.0],
            'layer_num': [2, 3, 4]
        }
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(dataset, labels)):
        ...
        for epoch in range(config.num_epochs):
            ...
            sampled_arch = {
                'encoder1': {
                    'dim_model': np.random.choice(arch_params['encoder1']['dim_model']),
                    'num_heads': np.random.choice(arch_params['encoder1']['num_heads']),
                    'mlp_ratio': np.random.choice(arch_params['encoder1']['mlp_ratio']),
                    'layer_num': np.random.choice(arch_params['encoder1']['layer_num'])
                },
                'encoder2': {
                    'dim_model': np.random.choice(arch_params['encoder2']['dim_model']),
                    'num_heads': np.random.choice(arch_params['encoder2']['num_heads']),
                    'mlp_ratio': np.random.choice(arch_params['encoder2']['mlp_ratio']),
                    'layer_num': np.random.choice(arch_params['encoder2']['layer_num'])
                },
                'encoder3': {
                    'dim_model': np.random.choice(arch_params['encoder3']['dim_model']),
                    'num_heads': np.random.choice(arch_params['encoder3']['num_heads']),
                    'mlp_ratio': np.random.choice(arch_params['encoder3']['mlp_ratio']),
                    'layer_num': np.random.choice(arch_params['encoder3']['layer_num'])
                },
                'encoder_multi': {
                    'dim_model': np.random.choice(arch_params['encoder_multi']['dim_model']),
                    'num_heads': np.random.choice(arch_params['encoder_multi']['num_heads']),
                    'mlp_ratio': np.random.choice(arch_params['encoder_multi']['mlp_ratio']),
                    'layer_num': np.random.choice(arch_params['encoder_multi']['layer_num'])
                }
            }
            
            # 更新模型架构
            model.update_architecture(sampled_arch)
            ...
            if val_acc > best_acc:
                best_acc = val_acc
                best_arch = sampled_arch  # 保存最佳架构
                torch.save({
                    'arch': best_arch,
                    'state_dict': model.state_dict(),
                    'accuracy': best_acc
                }, os.path.join(result_path, 'best_model_arch.pth'))
            print('Epoch: ', epoch,
