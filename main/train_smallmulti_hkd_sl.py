...
def train(save_all_checkpoint=False):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(dataset, labels)):
        selected = 0.0
        print('\n', '-' * 15, '>', f'Fold {fold}', '<', '-' * 15)
        if not os.path.exists('./saved/student_hkd_sl_Kfold_models/fold{}'.format(fold)):
            os.makedirs('./saved/student_hkd_sl_Kfold_models/fold{}'.format(fold))
        result_path = './saved/student_hkd_sl_Kfold_models/fold{}'.format(fold)
        ...
        #     torch.load('saved/student_hkd_sl_Kfold_models/fold0_pre/model.pkl'))
        ...
        # 下降为原来的一半
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [50,60,70,80], gamma=0.5, last_epoch=-1, verbose=False)
        ...
        for epoch in range(config.num_epochs):
         ...
         '''!!!'''
            ...
            early_stopping(val_acc, model, path='./saved/student_hkd_sl_Kfold_models/fold{}/model_{}_epoch{}.pkl'.format(fold, fold, epoch))
        ...
        if not os.path.exists('./result_excel/smallmulti_hkd_sl_2/fold{}'.format(fold)):
            os.makedirs('./result_excel/smallmulti_hkd_sl_2/fold{}'.format(fold))
        path = './result_excel/smallmulti_hkd_sl_2/fold{}/smodel_hkd_sl_fold{}.xlsx'.format(fold, fold)
...
if __name__ == '__main__':
    ...
    if not os.path.exists('./saved/student_hkd_sl_Kfold_models/best_fold'):
        os.makedirs('./saved/student_hkd_sl_Kfold_models/best_fold')
    with open("./saved/student_hkd_sl_Kfold_models/best_fold/best_fold.txt", 'a') as f:
