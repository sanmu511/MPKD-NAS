...
def train(save_all_checkpoint=False):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(dataset, labels)):
        print('\n', '-' * 15, '>', f'Fold {fold}', '<', '-' * 15)
        if not os.path.exists('./saved/teacher_Kfold_models/fold{}'.format(fold)):
            os.makedirs('./saved/teacher_Kfold_models/fold{}'.format(fold))
        ...
        for epoch in range(config.num_epochs):
            ...
            early_stopping(val_acc, model, path='./saved/teacher_Kfold_models/fold{}/tmodel_{}_epoch{}.pkl'.format(fold, fold, epoch))
