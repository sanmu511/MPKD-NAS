...
def train(save_all_checkpoint=False):
    config = Config()
    path = Path()
    caculate_metrics = Caculate_Metrics(config,path,'small_multi_nokd_Kfold_models')
    # tem calculate
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(dataset, labels)):
        ...
        print('\n', '-' * 15, '>', f'Fold {fold}', '<', '-' * 15)
        # 设置保存模型的地址
        if not os.path.exists('./saved/small_multi_nokd_Kfold_models/fold{}'.format(fold)):
            os.makedirs('./saved/small_multi_nokd_Kfold_models/fold{}'.format(fold))
        result_path = './saved/small_multi_nokd_Kfold_models/fold{}'.format(fold)
        ...
        for epoch in range(config.num_epochs):
            ...
            early_stopping(val_acc, model, path='./saved/small_multi_nokd_Kfold_models/fold{}/model_{}_epoch{}.pkl'.format(fold, fold, epoch))
        ...
        if not os.path.exists('./result_excel/small_multi_nokd/fold{}'.format(fold)):
            os.makedirs('./result_excel/small_multi_nokd/fold{}'.format(fold))
        path = './result_excel/small_multi_nokd/fold{}/small_nokd_fold{}.xlsx'.format(fold, fold)
...
if __name__ == '__main__':
    ...
    if not os.path.exists('saved/small_multi_nokd_Kfold_models/best_fold'):
        os.makedirs('saved/small_multi_nokd_Kfold_models/best_fold')
    with open("saved/small_multi_nokd_Kfold_models/best_fold/best_fold.txt", 'a') as f:
