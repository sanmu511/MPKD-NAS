...
from s_model_aux2_QUAN import s_Transformer_Auxiliary
...
def train(save_all_checkpoint=False):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(dataset, labels)):
        ...
        # 为backbone的每个encoder层设置量化配置
        model.backbone.transformer_encoder_1.qconfig = qconfig
        model.backbone.transformer_encoder_2.qconfig = qconfig
        model.backbone.transformer_encoder_3.qconfig = qconfig
        model.backbone.transformer_encoder_multi.qconfig = qconfig
        # 为辅助分类器设置量化配置
        model.auxiliary_classifier.transformer_encoder_multi.qconfig = qconfig
        # 融合模型层
        ...
        for epoch in range(config.num_epochs):
            ...
            if early_stopping.early_stop:
                ...
                # 转换为量化模型并评估性能
                model.eval()
                quantized_model = convert(model.cpu())
                # 验证量化后的性能
                quantized_model = quantized_model.to(config.device)
                test_acc_quantized, test_loss_quantized, _, __ = test(quantized_model, test_loader, config)
                print(f'Quantized model performance - test acc: {test_acc_quantized:.4f}, test loss: {test_loss_quantized:.4f}')
                
                # 评估每个encoder层的量化效果
                print("\nEvaluating individual encoder layers:")
                for name, module in quantized_model.backbone.named_children():
                    if isinstance(module, nn.TransformerEncoder):
                        print(f"Layer {name} quantization parameters:")
                        for param_name, param in module.state_dict().items():
                            if 'scale' in param_name or 'zero_point' in param_name:
                                print(f"  {param_name}: {param.mean().item():.4f}")
                
                # 保存量化模型和详细的量化指标
                torch.save(quantized_model.state_dict(), 
                    f'./saved/student_hkd_sl2_Kfold_models/fold{fold}/quantized_model.pth')
                
                # 记录量化前后的性能对比和详细指标
                with open(f'./saved/student_hkd_sl2_Kfold_models/fold{fold}/quantization_metrics.txt', 'w') as f:
                    f.write(f'Original model test accuracy: {test_acc:.4f}\n')
                    f.write(f'Quantized model test accuracy: {test_acc_quantized:.4f}\n')
                    f.write(f'Accuracy difference: {test_acc_quantized - test_acc:.4f}\n\n')
                    f.write('Encoder Layer Quantization Details:\n')
                    for name, module in quantized_model.backbone.named_children():
                        if isinstance(module, nn.TransformerEncoder):
                            f.write(f"\nLayer {name}:\n")
                            for param_name, param in module.state_dict().items():
                                if 'scale' in param_name or 'zero_point' in param_name:
                                    f.write(f"  {param_name}: {param.mean().item():.4f}\n")
                break
        # 记录每个fold的最佳准确率
